...
interface CartStore {
  items: CartItem[];
  addItem: (product: Product, quantity: number, options?: Record<string, string>, meta?: { estimated_kg?: number; estimated_cbm?: number }) => void;
        ...
        if (existing) {
          set({
            items: get().items.map((i) =>
              i.id === existing.id
                ? { ...i, quantity: i.quantity + quantity, estimated_kg: meta.estimated_kg ?? i.estimated_kg, estimated_cbm: meta.estimated_cbm ?? i.estimated_cbm }
                : i
            ),
          });
        } else {
